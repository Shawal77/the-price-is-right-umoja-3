...
class Deal extends React.Component {
  ...
  async guessedPrice() { // Fun([], UInt)we provide the guessedPrice function callback.
    const priceGuess = await new Promise(resolvepriceGuessP => {//we set the component state to display Get priceGuess dialog, and wait for a Promise which can be resolved via user interaction.
      this.setState({view: 'GetpriceGuess', playable: true, resolvepriceGuessP});
    });
  ...
  seeResult(i) { this.setState({view: 'Done', outcome: intToOutcome[i]}); }//we provide the seeOutcome and informTimeout callbacks, which set the component state to display Done display and Timeout display, respectively.3
  informTimeout() { this.setState({view: 'Timeout'}); }
  playpriceGuess(priceGuess) { this.state.resolvepriceGuessP(priceGuess); }//On line 53, we define what happens when the user guesses a number The Promise from line 45 is resolved
